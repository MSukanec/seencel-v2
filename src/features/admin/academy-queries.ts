...
export async function getAdminCourses(): Promise<AdminCourse[]> {
    ...
    return (courses || []).map((c) => {
        const details = c.course_details as any;
...
export async function getCoursesDashboardData(): Promise<CoursesDashboardData> {
    ...
    const { data: payments } = await supabase
        .schema('billing').from('payments')
        .select('amount, currency, course_id, created_at, status')
        .not('course_id', 'is', null)
        .eq('status', 'completed');
    ...
    const { data: progressData } = await supabase
        .schema('academy').from('course_progress_view')
        .select('user_id, course_id, progress_pct');
    ...
    const courseStats = (courses || []).map(c => {
        ...
        const coursePayments = payments?.filter(p => p.course_id === c.id) || [];
        const courseProgress = progressData?.filter(p => p.course_id === c.id) || [];
        const courseAvgProgress = courseProgress.length > 0
            ? courseProgress.reduce((sum, p) => sum + (p.progress_pct || 0), 0) / courseProgress.length
            : 0;
        return {
            course_id: c.id,
            course_title: c.title,
            enrollments: courseEnrollments.length,
            revenue: coursePayments.reduce((sum, p) => sum + (p.amount || 0), 0),
