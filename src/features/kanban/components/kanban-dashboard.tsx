...
import { KanbanBoard, PRIORITY_CONFIG } from "@/features/kanban/types";
import { useLayoutStore } from "@/store/layout-store";
import { KanbanBoard as KanbanBoardComponent } from "@/features/kanban/components/kanban-board";
...
import { deleteBoard } from "@/features/kanban/actions";
...
export function KanbanDashboard({
    ...
    const handleBoardSwitch = (boardId: string) => {
        ...
        setIsSwitching(true);
        router.push(`/organization/kanban?boardId=${boardId}`);
    };
        ...
        openModal(
            <KanbanBoardForm
                ...
                onSuccess={(newBoard: KanbanBoard) => {
                    closeModal();
                    router.push(`/organization/kanban?boardId=${newBoard.id}`);
                }}
            />,
            {
                title: "Nuevo Tablero",
                        ...
                        onClick={async () => {
                            try {
                                ...
                                if (remaining.length > 0) {
                                    router.push(`/organization/kanban?boardId=${remaining[0].id}`);
                                } else {
                                    router.push(`/organization/kanban`);
                                }
                            } catch (error) {
