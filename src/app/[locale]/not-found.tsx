...
export default function NotFound() {
    ...
    // Simple check to determine if we are likely in a dashboard context to suggest going there
    const isDashboard = pathname.includes('/project') || pathname.includes('/organization');
            ...
            <div className="space-y-6 max-w-md w-full">
                ...
                <div className="flex flex-col sm:flex-row gap-2 justify-center pt-4">
                    <Button asChild variant="default" size="lg">
                        <Link href={isDashboard ? "/organization" : "/"}>
                            {isDashboard ? <LayoutDashboard className="mr-2 h-4 w-4" /> : <Home className="mr-2 h-4 w-4" />}
                            {isDashboard ? t('goDashboard') : t('goHome')}
                        </Link>
                    </Button>
