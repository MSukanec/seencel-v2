...
export default async function ProjectFinancePage({ params }: { params: Promise<{ locale: string; projectId: string }> }) {
    ...
    // Build wallets array from settings - use w.id (organization_wallets.id) not wallet_id
    const wallets = (settingsData.contactWallets || []).map((w: any) => ({
        id: w.id,
                ...
                <ContentLayout variant="wide">
                    ...
                    <TabsContent value="payments" className="m-0 h-full focus-visible:outline-none">
                        <FinancesMovementsView
                            ...
                            currencies={(settingsData.contactCurrencies || []).map((c: any) => ({
