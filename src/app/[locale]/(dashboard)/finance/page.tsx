...
export default async function FinancePage() {
    ...
    if (error || !movements) {
        return <div className="p-8 text-center text-red-500">Error loading financial data: {error}</div>;
    }
    ...
    const totalExpense = movements.reduce((acc: number, m: any) => {
        const sign = Number(m.amount_sign ?? 1);
        return sign < 0 ? acc + Number(m.amount) : acc; // amount is standardly positive in DB often, but here we sum the magnitude
    }, 0);
        ...
        <div className="space-y-8 pb-20 animate-in fade-in duration-700">
            ...
            <div>
                ...
                <p className="text-muted-foreground mt-1">
                    Vista unificada de todos los movimientos de la organización.
                </p>
            </div>
