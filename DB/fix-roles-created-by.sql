-- ============================================
-- Fix: handle_new_organization
-- 
-- 1. Borrar la versión vieja (3 params)
-- 2. Recrear la versión nueva (4 params) 
--    SIN created_by en iam.roles
-- ============================================

-- Paso 1: Borrar la versión vieja (3 params, sin p_default_currency_id)
DROP FUNCTION IF EXISTS iam.handle_new_organization(uuid, text, text);

-- Paso 2: Recrear la versión nueva corregida (4 params)
CREATE OR REPLACE FUNCTION iam.handle_new_organization(
    p_user_id uuid,
    p_organization_name text,
    p_business_mode text DEFAULT 'professional'::text,
    p_default_currency_id uuid DEFAULT NULL::uuid
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    -- Step 3: Roles por defecto (SIN created_by — la tabla roles no tiene esa columna)
    INSERT INTO iam.roles (organization_id, name, description, type, is_system)
    VALUES (v_organization_id, 'Administrador', 'Control total sobre la organización', 'organization', true)
    RETURNING id INTO v_admin_role_id;

    INSERT INTO iam.roles (organization_id, name, description, type, is_system)
    VALUES (v_organization_id, 'Editor', 'Puede editar contenido', 'organization', true)
    RETURNING id INTO v_editor_role_id;

    INSERT INTO iam.roles (organization_id, name, description, type, is_system)
    VALUES (v_organization_id, 'Lector', 'Solo lectura', 'organization', true)
    RETURNING id INTO v_reader_role_id;

    -- Step 4: Agregar creador como miembro administrador
    INSERT INTO iam.organization_members (user_id, organization_id, role_id)
    VALUES (p_user_id, v_organization_id, v_admin_role_id)
    ...
    -- Step 9: Board Kanban por defecto
