...
- **`iam.organization_clients`** (10 cols | FK: organization_id → organizations, user_id → users, created_by → organization_members, updated_by → organization_members)
...
- **`iam.organization_external_actors`** (11 cols | FK: created_by → organization_members, updated_by → organization_members, user_id → users, organization_id → organizations)
- **`iam.organization_invitations`** (16 cols | FK: role_id → roles, user_id → users, invited_by → organization_members, organization_id → organizations)
- **`iam.organization_members`** (14 cols | FK: updated_by → organization_members, organization_id → organizations, invited_by → organization_members, role_id → roles, created_by → organization_members, user_id → users)
...
- **`projects.project_clients`** (14 cols | FK: client_role_id → client_roles, project_id → projects)
...
- **`finance.client_payments`** (21 cols | FK: currency_id → currencies, commitment_id → client_commitments, schedule_id → client_payment_schedule, wallet_id → organization_wallets)
...
- **`finance.financial_operation_movements`** (15 cols | FK: currency_id → currencies, financial_operation_id → financial_operations, wallet_id → organization_wallets)
...
- **`finance.general_costs_payments`** (18 cols | FK: wallet_id → organization_wallets, currency_id → currencies, general_cost_id → general_costs)
...
- **`finance.movements`** (22 cols | FK: type_id → movement_concepts, currency_id → currencies, category_id → movement_concepts, subcategory_id → movement_concepts, wallet_id → organization_wallets)
...
- **`catalog.labor_types`** (9 cols | FK: labor_level_id → labor_levels, labor_category_id → labor_categories, labor_role_id → labor_roles, unit_id → units)
...
- **`catalog.task_parameter_options`** (14 cols | FK: parameter_id → task_parameters, unit_id → units, material_id → materials)
...
- **`catalog.task_recipe_materials`** (16 cols | FK: material_id → materials, recipe_id → task_recipes, unit_id → units)
...
- **`catalog.task_template_parameters`** (8 cols | FK: template_id → task_templates, parameter_id → task_parameters)
- **`catalog.task_templates`** (17 cols | FK: task_division_id → task_divisions, unit_id → units, task_action_id → task_actions, task_element_id → task_elements, task_construction_system_id → task_construction_systems)
- **`catalog.tasks`** (24 cols | FK: task_action_id → task_actions, task_construction_system_id → task_construction_systems, task_division_id → task_divisions, template_id → task_templates, task_element_id → task_elements, unit_id → units)
...
- **`planner.items`** (38 cols | FK: board_id → boards, parent_item_id → items, list_id → lists)
