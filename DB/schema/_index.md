...
- **`labor_prices`** (11 cols | FK: organization_id → organizations, created_by → organization_members, currency_id → currencies, updated_by → organization_members, labor_type_id → labor_types)
- **`labor_roles`** (10 cols)
- **`labor_types`** (9 cols | FK: unit_id → units, labor_level_id → labor_levels, labor_category_id → labor_categories, labor_role_id → labor_roles)
- **`linked_accounts`** (5 cols | FK: user_id → users)
- **`material_categories`** (5 cols | FK: parent_id → material_categories)
- **`material_invoice_items`** (13 cols | FK: material_id → materials, invoice_id → material_invoices, unit_id → units, project_id → projects, organization_id → organizations, product_id → products)
- **`material_invoices`** (18 cols | FK: organization_id → organizations, purchase_order_id → material_purchase_orders, project_id → projects, currency_id → currencies, provider_id → contacts, created_by → organization_members)
- **`material_payments`** (20 cols | FK: currency_id → currencies, material_type_id → material_types, created_by → organization_members, import_batch_id → import_batches, organization_id → organizations, project_id → projects, purchase_id → material_invoices, updated_by → organization_members, wallet_id → organization_wallets)
...
- **`material_purchase_orders`** (18 cols | FK: currency_id → currencies, approved_by → organization_members, project_id → projects, provider_id → contacts, requested_by → organization_members, organization_id → organizations)
- **`material_types`** (11 cols | FK: updated_by → organization_members, organization_id → organizations, created_by → organization_members)
- **`materials`** (20 cols | FK: organization_id → organizations, default_provider_id → contacts, category_id → material_categories, created_by → organization_members, updated_by → organization_members, unit_id → units, default_sale_unit_id → units)
...
- **`ops_alerts`** (20 cols | FK: payment_id → payments, ack_by → users, user_id → users, resolved_by → users, organization_id → organizations, event_id → payment_events)
...
- **`organization_data`** (22 cols | FK: updated_by → organization_members, created_by → organization_members, organization_id → organizations)
...
- **`organization_invitations`** (14 cols | FK: user_id → users, role_id → roles, organization_id → organizations, invited_by → organization_members)
...
- **`organization_members`** (14 cols | FK: created_by → organization_members, invited_by → organization_members, role_id → roles, organization_id → organizations, user_id → users, updated_by → organization_members)
...
- **`organization_recipe_preferences`** (5 cols | FK: recipe_id → task_recipes, organization_id → organizations, task_id → tasks)
...
- **`organizations`** (16 cols | FK: created_by → users, updated_by → organization_members, plan_id → plans, owner_id → users)
- **`partner_capital_balance`** (9 cols | FK: organization_id → organizations, partner_id → capital_participants)
- **`partner_contributions`** (17 cols | FK: currency_id → currencies, wallet_id → organization_wallets, organization_id → organizations, partner_id → capital_participants, project_id → projects, created_by → organization_members)
...
- **`permissions`** (6 cols)
- **`personnel_attendees`** (13 cols | FK: personnel_id → project_labor, organization_id → organizations, site_log_id → site_logs, project_id → projects, created_by → organization_members)
...
- **`project_clients`** (14 cols | FK: created_by → organization_members, updated_by → organization_members, project_id → projects, organization_id → organizations, client_role_id → client_roles, contact_id → contacts)
...
- **`project_labor`** (15 cols | FK: project_id → projects, contact_id → contacts, created_by → organization_members, labor_type_id → labor_categories, updated_by → organization_members, organization_id → organizations)
...
- **`projects`** (19 cols | FK: organization_id → organizations, created_by → organization_members, updated_by → organization_members, project_type_id → project_types, project_modality_id → project_modalities)
- **`provider_products`** (7 cols | FK: organization_id → organizations, product_id → products)
- **`push_subscriptions`** (7 cols | FK: user_id → users)
- **`quote_items`** (19 cols | FK: project_id → projects, quote_id → quotes, task_id → tasks, updated_by → organization_members, created_by → organization_members, currency_id → currencies, organization_id → organizations)
- **`quotes`** (27 cols | FK: organization_id → organizations, client_id → contacts, created_by → organization_members, updated_by → organization_members, project_id → projects, currency_id → currencies, approved_by → organization_members, parent_quote_id → quotes)
...
- **`site_log_types`** (11 cols | FK: organization_id → organizations, updated_by → organization_members, created_by → organization_members)
- **`site_logs`** (20 cols | FK: project_id → projects, updated_by → organization_members, created_by → organization_members, entry_type_id → site_log_types, organization_id → organizations)
- **`subcontract_bid_tasks`** (10 cols | FK: subcontract_task_id → subcontract_tasks, subcontract_bid_id → subcontract_bids)
- **`subcontract_bids`** (12 cols | FK: created_by → organization_members, currency_id → currencies, subcontract_id → subcontracts, contact_id → contacts)
...
- **`subcontracts`** (22 cols | FK: organization_id → organizations, created_by → organization_members, currency_id → currencies, project_id → projects, adjustment_index_type_id → economic_index_types, contact_id → contacts, updated_by → organization_members, winner_bid_id → subcontract_bids)
...
- **`task_divisions`** (15 cols | FK: organization_id → organizations, updated_by → organization_members, parent_id → task_divisions, import_batch_id → import_batches, created_by → organization_members)
...
- **`task_recipe_external_services`** (17 cols | FK: contact_id → contacts, created_by → organization_members, currency_id → currencies, import_batch_id → import_batches, organization_id → organizations, recipe_id → task_recipes, unit_id → units, updated_by → organization_members)
...
- `current_user_id()` → uuid 🔐 *(functions_1.md)*
- `dismiss_home_banner()` → boolean 🔐 *(functions_1.md)*
- `documents_validate_project_org()` → trigger 🔐 *(functions_1.md)*
- `fill_progress_user_id_from_auth()` → trigger 🔐 *(functions_1.md)*
...
- `handle_contact_link_user()` → trigger 🔐 *(functions_2.md)*
...
- `handle_registered_invitation()` → trigger 🔐 *(functions_2.md)*
...
- `heartbeat(p_org_id uuid, p_status text DEFAULT 'online'::text, p_session_id uuid DEFAULT NULL::uuid)` → void 🔐 *(functions_3.md)*
...
- `kanban_set_updated_at()` → trigger *(functions_3.md)*
- `lock_org_task_on_update()` → trigger 🔐 *(functions_3.md)*
- `log_activity(p_organization_id uuid, p_user_id uuid, p_action text, p_target_table text, p_target_id uuid, p_metadata jsonb)` → void 🔐 *(functions_3.md)*
...
- `log_kanban_card_activity()` → trigger 🔐 *(functions_4.md)*
- `log_kanban_child_activity()` → trigger 🔐 *(functions_4.md)*
- `log_kanban_comment_activity()` → trigger 🔐 *(functions_4.md)*
- `log_kanban_label_activity()` → trigger 🔐 *(functions_4.md)*
...
- `log_quote_item_activity()` → trigger 🔐 *(functions_5.md)*
- `log_recipe_external_service_activity()` → trigger 🔐 *(functions_5.md)*
- `log_recipe_labor_activity()` → trigger 🔐 *(functions_5.md)*
- `log_recipe_material_activity()` → trigger 🔐 *(functions_5.md)*
...
- `notify_new_transfer()` → trigger 🔐 *(functions_6.md)*
- `notify_new_user()` → trigger 🔐 *(functions_6.md)*
- `notify_push_on_notification()` → trigger 🔐 *(functions_6.md)*
- `notify_quote_status_change()` → trigger 🔐 *(functions_6.md)*
- `notify_subscription_activated()` → trigger 🔐 *(functions_6.md)*
...
- `recalculate_po_totals()` → trigger 🔐 *(functions_7.md)*
- `recalculate_recipe_rating()` → trigger *(functions_7.md)*
- `redeem_coupon_universal(p_code text, p_product_type text, p_product_id uuid, p_price numeric, p_currency text DEFAULT 'USD'::text, p_order_id uuid DEFAULT NULL::uuid, p_subscription_id uuid DEFAULT NULL::uuid)` → jsonb 🔐 *(functions_7.md)*
- `refresh_labor_avg_prices()` → void 🔐 *(functions_7.md)*
- `refresh_material_avg_prices()` → void 🔐 *(functions_7.md)*
- `refresh_product_avg_prices()` → void 🔐 *(functions_7.md)*
...
- `step_create_organization_roles(p_org_id uuid)` → jsonb 🔐 *(functions_8.md)*
- `step_create_organization_wallets(p_org_id uuid, p_wallet_id uuid)` → void 🔐 *(functions_8.md)*
- `step_create_user(p_auth_user_id uuid, p_email text, p_full_name text, p_avatar_url text, p_avatar_source avatar_source_t, p_role_id uuid)` → uuid 🔐 *(functions_8.md)*
- `step_create_user_acquisition(p_user_id uuid, p_raw_meta jsonb)` → void 🔐 *(functions_8.md)*
- `step_create_user_data(p_user_id uuid)` → void 🔐 *(functions_8.md)*
- `step_create_user_organization_preferences(p_user_id uuid, p_org_id uuid)` → void 🔐 *(functions_8.md)*
...
- `unaccent_init(internal)` → internal *(functions_9.md)*
- `unaccent_lexize(internal, internal, internal, internal)` → internal *(functions_9.md)*
- `update_contact_category_links_updated_at()` → trigger *(functions_9.md)*
