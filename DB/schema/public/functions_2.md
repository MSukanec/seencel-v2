...
## [PUBLIC] Functions (chunk 2: documents_validate_project_org — handle_new_user)
...
### `get_next_change_order_number(p_contract_id uuid)`
...
```sql
CREATE OR REPLACE FUNCTION public.get_next_change_order_number(p_contract_id uuid)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$
DECLARE
