-- =============================================
-- admin_cleanup_test_purchase - Sin restricciones de whitelist
-- Permite limpiar CUALQUIER usuario admin desde la página de soporte
-- =============================================

-- Primero borrar la función existente
DROP FUNCTION IF EXISTS admin_cleanup_test_purchase(TEXT, UUID);
...
AS $$
DECLARE
    v_user_id UUID;          -- This will now be users.id (internal ID)
...
BEGIN
    -- Usar el org_id pasado como parámetro
    v_org_id := p_org_id;
    ...
    -- 1. Borrar course_enrollments (uses users.id)
    ...
    -- 8. Borrar paypal preferences del usuario
