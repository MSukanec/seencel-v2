-- FIX: admin_cleanup_test_purchase was using auth.users.id instead of users.id
-- course_enrollments.user_id expects the internal users.id, not auth_id
...
DECLARE
    ...
    v_auth_user_id UUID;     -- auth.users.id for validation only
...
BEGIN
    ...
    -- FIXED: Get from public.users table, not auth.users
    -- ============================================
    ...
    -- FIXED: Get the internal users.id, not auth.users.id
