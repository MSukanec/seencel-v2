...
create index IF not exists idx_client_commitments_functional_amount on public.client_commitments using btree (functional_amount) TABLESPACE pg_default;

create trigger client_commitments_set_updated_at BEFORE
update on client_commitments for EACH row
execute FUNCTION update_timestamp ();

create trigger on_client_commitment_audit
after INSERT
or DELETE
or
update on client_commitments for EACH row
execute FUNCTION log_client_commitment_activity ();

create trigger set_audit_client_commitments BEFORE INSERT
or
update on client_commitments for EACH row
...
with
  commitment_totals as (
    select
      ...
      sum(client_commitments.functional_amount) as total_functional_committed
    from
      client_commitments
    where
  ...
  payment_totals as (
    select
      ...
      sum(client_payments.functional_amount) as total_functional_paid
    from
      client_payments
    where
...
select
  ...
  COALESCE(ct.total_functional_committed, 0::numeric) as total_functional_committed_amount,
  COALESCE(pt.total_functional_paid, 0::numeric) as total_functional_paid_amount,
  COALESCE(ct.total_functional_committed, 0::numeric) - COALESCE(pt.total_functional_paid, 0::numeric) as functional_balance_due
from
  project_clients pc
...
create table public.client_payments (
  ...
  constraint client_payments_updated_by_fkey foreign KEY (updated_by) references organization_members (id) on delete set null,
...
execute FUNCTION log_client_payment_activity ();

create trigger set_audit_client_payments BEFORE INSERT
or
update on client_payments for EACH row
execute FUNCTION handle_updated_by ();
